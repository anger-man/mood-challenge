...
def get_subject_globpred(data, target):
    ...
    up = nn.Upsample(scale_factor = 64)
...
valid_loader = DataLoader(vali_dataset, batch_size=4, shuffle=True)
...
for epoch in range(1, n_epochs+1):
    ...
    for data, target in bar:
        ...
        patches_queue = tio.data.Queue(subjects_dataset=subjects_dataset, max_length = 48, samples_per_volume= 12, sampler = sampler)
        ...
        for patch_subject in patches_loader:
            ...
            if use_global:
                patch_global = patch_subject['pred_global_up'][tio.DATA]
                patch_data = torch.cat((patch_data, patch_global))
    ...
    with torch.no_grad(): #deactivates the autograd engine
        ...
        for data, target in bar:
            ...
            sampler = tio.data.LabelSampler(patch_size = 64, label_probabilities = {0:3, 1:7})
            patches_eval_queue = tio.data.Queue(subjects_dataset=subjects_eval_dataset, max_length=48, samples_per_volume=12, sampler=sampler)
    ...
    for data, target in tq(test_loader):
        ...
        for patch_subject in generator:
            ...
            if use_global:
                patch_global =  patch_subject['pred_global_up'].data
