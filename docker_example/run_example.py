...
if __name__ == "__main__":
    ...
    output_dir_sample = '/home/c/mood/data/mood_preds/sample'
    output_dir_pixel = '/home/c/mood/data/mood_preds/pixel'
    ...
    try:
        ...
        ret = subprocess.run(["sudo","docker", "build", example_dir, "-t", "mood_example"], check=True)
    except Exception:
    ...
    try:
        docker_str = (
            ...
            f"-v {output_dir_pixel}:/mnt/pred mood_example sh /workspace/run_pixel_brain.sh /mnt/data /mnt/pred"
        )
    ...
    try:
        docker_str = (
            f"sudo docker run {gpu_str}-v {toy_input_dir}:/mnt/data "
            f"-v {output_dir_sample}:/mnt/pred mood_example sh /workspace/run_sample_brain.sh /mnt/data /mnt/pred"
        )
        ret = subprocess.run(docker_str.split(" "), check=True,)
    except Exception:
    ...
    res_pixel = evalresults.eval_dir(output_dir_pixel, toy_label_pixel_dir, mode="pixel",)
    print("Pixel-level score:", res_pixel)

    res_sample = evalresults.eval_dir(output_dir_sample, toy_label_sample_dir, mode="sample",)
